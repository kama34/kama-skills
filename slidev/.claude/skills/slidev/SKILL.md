...
**КРИТИЧНО: НИКОГДА не вызывать параллельно.** Один вызов `generate_screen_from_text` за раз. Дождаться результата, только потом следующий. Параллельные вызовы приводят к потере всех результатов.

1. Вызвать `generate_screen_from_text`. Вызов вернётся СРАЗУ (не блокирует). Это нормально — генерация продолжается на сервере.
2. Если вернулся результат с `output_components` → экран готов, вызвать `get_screen`.
3. Если вернулся пустой ответ (completed with no output) → генерация ещё идёт. **ЖДАТЬ:**
   - Подождать **120 секунд** (2 минуты) → `list_screens` → если экран появился → `get_screen`
   - Если нет → подождать ещё **120 секунд** → `list_screens` → проверить
   - Повторить до **5 попыток** (суммарно до 10 минут ожидания)
   - Простой промпт = ~2 минуты. Сложный промпт (полная презентация) = 3-5 минут.
4. Если после 5 попыток экран не появился → **ОСТАНОВИТЬ работу**. Напечатать: "Stitch API не ответил после 10 минут. Завершаю."
